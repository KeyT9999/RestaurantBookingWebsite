...
class CustomerChatManager {
    ...
    displayRestaurants(restaurants) {
        ...
        if (restaurants.length === 0) {
            restaurantList.innerHTML = '<div class="text-center text-muted"><small>Không có nhà hàng nào</small></div>';
            return;
        }
    ...
    handleIncomingMessage(data) {
        ...
        const loadingState = messagesContainer.querySelector('.text-center.text-muted');
        if (loadingState) {
            loadingState.remove();
        }
