-- =====================================================
-- SQL Script: XÓA NHÀ HÀNG "AVVVV" VÀ TẤT CẢ DỮ LIỆU LIÊN QUAN
-- =====================================================
-- 
-- Script này sẽ xóa nhà hàng có tên "AVVVV" và tất cả dữ liệu liên quan:
...
BEGIN
    ...
    IF v_restaurant_id IS NULL THEN
        RAISE NOTICE '❌ Không tìm thấy nhà hàng có tên "AVVVV"';
        RETURN;
    END IF;
    
    RAISE NOTICE '🔍 Tìm thấy nhà hàng "AVVVV" với ID: %', v_restaurant_id;
    ...
    IF v_deleted_count > 0 THEN
        ...
        RAISE NOTICE '   Đã xóa thành công nhà hàng "AVVVV" (ID: %) và tất cả dữ liệu liên quan', v_restaurant_id;
    ELSE
        RAISE NOTICE '';
...
SELECT 
    CASE 
        ...
        THEN '❌ Nhà hàng "AVVVV" vẫn còn tồn tại trong database!'
        ELSE '✅ Nhà hàng "AVVVV" đã được xóa thành công!'
